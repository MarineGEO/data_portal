...
shinyApp(
  ...
  server = function(input, output, session) {
    ...
    qualityAssuranceTest <- function() {
      df <- read.csv(input$fileCSV$datapath)
      
      site_columns <- c("study_id","site_id", "site_description", "site_latitude_max", 
                        "site_latitude_min", "site_longitude_max", "site_longitude_min",
                        "site_boundaries", "salinity_class", "salinity_method", "salinity_notes",
                        "vegetation_class", "vegetation_method", "vegetation_notes",
                        "inundation_class","inundation_method","inundation_notes")
      
      column_names <- colnames(df)
      non_matching_columns <- subset(column_names, !(column_names %in% site_columns))
